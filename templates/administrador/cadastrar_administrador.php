...
<?php  
	if(count($_POST) > 0){
		...
		if($pessoa){ 
			...
			$administrador = run_select('select * from administrador where idfuncionario='.$funcionario['idfuncionario'].';');
			if(!$administrador){
				$administrador = array();
				$administrador['idfuncionario'] = $funcionario['idfuncionario'];
				...
				swal('Erro!', 'Essa pessoa já está cadastrada como um administrador', 'error', ADMINISTRADOR.'/cadastrar_administrador', 'btn-danger');
			}
		}
		else{
			/* CADASTRO PESSOA */
			...
			$administrador = array();
			$administrador['idfuncionario'] = $idfuncionario['max(idfuncionario)'];
