...
<?php  
	if(isset($_SESSION['idpessoa'])):
		...
		if(isset($_SESSION['idpaciente'])){
			...
			$agenda = run_select_many('select hora, data, preco, p1.nome as nome_paciente, p2.nome as nome_medico from pessoa as p1 natural join paciente, agenda natural join consulta, pessoa as p2 natural join funcionario as f2 natural join medico where '.$pessoa.'.email=\''.$_POST['email'].'\' and consulta.data > \''.date('Y-m-d').'\' and paciente.idpaciente=agenda.idpaciente and medico.idmedico=agenda.idmedico order by data;');
		}
?>
